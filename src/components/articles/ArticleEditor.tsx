  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      editor.state.doc.descendants((node: any) => {
        if (node.type.name === "imageWithRemove") {
      ...
      for (const url of uploadedImagesRef.current) {
        if (!currentImages.has(url)) {
          fetch("/api/delete-asset", {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ url }),
          }).then(() => uploadedImagesRef.current.delete(url))
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2 p-2 border rounded bg-white" style={{ borderColor: "#D8CDBE" }}>
        ...
        <button onClick={() => editor.chain().focus().toggleBulletList().run()} className={`px-2 py-1 rounded ${editor.isActive("bulletList") ? "bg-[#E6DCCB]" : "bg-white"} !font-sans`}>• List</button>
        <button onClick={() => editor.chain().focus().toggleOrderedList().run()} className={`px-2 py-1 rounded ${editor.isActive("orderedList") ? "bg-[#E6DCCB]" : "bg-white"} !font-sans`}>1. List</button>
